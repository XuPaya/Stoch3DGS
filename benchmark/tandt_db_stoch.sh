...
RESULT_DIR=${RESULT_DIR:-"results/tandt_db_stoch_full_15spp"}
...
SCENE_LIST="train truck drjohnson playroom"

for SCENE in $SCENE_LIST;
do
    if [ "$SCENE" = "train" ] || [ "$SCENE" = "truck" ]; then
        DATASET="tandt"
    else
        DATASET="db"
    fi
    ...
    CUDA_VISIBLE_DEVICES=0 python train.py --config-name $CONFIG \
        ...
        path=data/$DATASET/$SCENE experiment_name=$SCENE \
