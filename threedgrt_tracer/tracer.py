...
class Tracer:
    class _Autograd(torch.autograd.Function):
        ...
        def forward(
            ...
            ray_radiance, ray_density, ray_hit_distance, ray_normals, hits_count, mog_visibility, mog_weight, ray_sampleCache = tracer_wrapper.trace(
                frame_id,
                ray_to_world,
                ray_ori,
                ray_dir,
        ...
        @staticmethod
        def backward(
            ctx, ray_radiance_grd, ray_density_grd, ray_hit_distance_grd, ray_normals_grd, ray_hits_count_grd_UNUSED, mog_visibility_grd_UNUSED, mog_weight_grd_UNUSED
        ):
            (
                ray_to_world,
                ray_ori,
            ...
            particle_density_grd, mog_sph_grd, ray_grad_vis_grd = ctx.tracer_wrapper.trace_bwd(
                frame_id,
                ray_to_world,
                ray_ori,
                ray_dir,
    ...
    def render(self, gaussians, gpu_batch: Batch, train=False, frame_id=0):
        ...
        with torch.cuda.nvtx.range(f"model.forward({num_gaussians} gaussians)"):
            ...
            (pred_rgb, pred_opacity, pred_dist, pred_normals, hits_count, mog_visibility, mog_weight) = Tracer._Autograd.apply(
                self.tracer_wrapper,
                frame_id,
