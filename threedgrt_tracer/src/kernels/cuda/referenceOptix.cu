...
extern "C" __global__ void __raygen__rg() {
    ...
    RayPayload rayPayload;
    float sampleDensity[64];
    #pragma unroll
    for (int h = 0; h < 64; h++) {
        sampleDensity[h] = -1.0f;
    }
...
#pragma unroll
        for (int i = 0; i < PipelineParameters::MaxNumHitPerTrace; i++) {
            ...
            if ((rayHit.particleId != RayHit::InvalidParticleId) && (rayTransmittance > params.minTransmittance)) {
                ...
                if (acceptedHit) {
                    ...
                    // For gaussian number clamping, the least contributing particles will be dumped.
                    // Remove the following condition if clamping is not needed.
                    if (global_idx < 64) {
                        sampleDensity[global_idx] = getDensityStochastic<PipelineParameters::ParticleKernelDegree, PipelineParameters::SurfelPrimitive>(
                            rayOrigin,
                            rayDirection,
                            rayHit.particleId,
                            params.particleDensity
    ...
    // store sample density cache
    #pragma unroll
    for (int h = 0; h < 64; h++) {
        params.raySampleCache[idx.z][idx.y][idx.x][h] = sampleDensity[h];
    }
