...
extern "C" __global__ void __raygen__rg() {
    ...
    float* raySampleCache     = &params.raySampleCache[idx.z][idx.y][idx.x][0];

    float gradVis = 0.f;
    params.rayGradVis[idx.z][idx.y][idx.x][0] = 0.0f;

#pragma unroll
    for (int spp = 0; spp < 1; spp++)  // spp_fwd samples
    {
...
#pragma unroll
        for (int it = 0; it < 8; it++) {
            int32_t residualParticleId = __float_as_uint(raySampleCache[it * 2 + 1]);
            int32_t particleId = __float_as_uint(raySampleCache[it * 2]);
...
extern "C" __global__ void __anyhit__ah() { // sorting removed
    ...
    #define STOCHASTIC_HIT_PAYLOAD_BWD(w_hit_id, w_distance, r_distance)                                      \
        {                                                                                 \
            ...
            seed = 1664525u * seed + 1013904223u;                                         \
            rand1d = (seed & 0x00FFFFFF) / float(0x01000000);                       \
            if (hit.distance > fin_distance + 1e-3 && hit.distance < curr_distance && rand1d < density && density > params.alphaMinThreshold) { \
