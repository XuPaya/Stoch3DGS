...
extern "C" __global__ void __raygen__rg() {
    ...
    int32_t samplePID1[8];
    int32_t samplePID2[8];
    #pragma unroll
    for (int h = 0; h < 8; h++) {
        samplePID1[h] = RayHit::InvalidParticleId;
        samplePID2[h] = RayHit::InvalidParticleId;
    }
...
#pragma unroll
        for (int i = 0; i < PipelineParameters::MaxNumHitPerTrace; i++) {
            ...
            if ((rayHit.particleId != RayHit::InvalidParticleId) && (rayTransmittance > params.minTransmittance)) {
                ...
                float density = getDensityStochastic<PipelineParameters::ParticleKernelDegree, PipelineParameters::SurfelPrimitive>(
                                    ...
                                    params.particleDensity);
                unsigned int seed = ((idx.x * 73856093u) ^ (idx.y * 19349663u) ^ (idx.z * 83492791u) ^ rayHit.particleId ^ params.frameNumber * 2654435761u);
                #pragma unroll
                for (int h = 0; h < 8; h++) {
                    seed = 1664525u * seed + 1013904223u;
                    float rand1d = (seed & 0x00FFFFFF) / float(0x01000000);
                    // if (samplePID1[h] >= 0) {
                    //     continue;
                    // }
                    // if (rand1d < density) {
                    //     samplePID[h] = rayHit.particleId;
                    //     sampleDistance[h] = rayHit.distance;
                    // }
                    if (samplePID1[h] == RayHit::InvalidParticleId) {
                        if (rand1d < density) {
                            samplePID1[h] = rayHit.particleId;
                        }
                    } else if (samplePID2[h] == RayHit::InvalidParticleId) {
                        if (rand1d < density) {
                            samplePID2[h] = rayHit.particleId;
                        }
                    }
                }
                ...
                atomicAdd(&params.particleWeight[rayHit.particleId], particleWeight);
    ...
    #pragma unroll
    for (int h = 0; h < 8; h++) {
        params.raySampleCache[idx.z][idx.y][idx.x][h * 2] = __uint_as_float(samplePID1[h]);
        params.raySampleCache[idx.z][idx.y][idx.x][h * 2 + 1] = __uint_as_float(samplePID2[h]);
    }
