...
class MixtureOfGaussians(torch.nn.Module):
    ...
    def __init__(self, conf, scene_extent=None):
        ...
        self.rolling_weight_contrib = torch.nn.Parameter(torch.empty([0, 1]))  # For num_of_gaussian capping: rolling optimization weight contribution
    ...
    def validate_fields(self):
        ...
        assert self.rolling_weight_contrib.shape == (num_gaussians, 1)
    ...
    def init_from_pretrained_point_cloud(self, pc_path: str, set_optimizable_parameters: bool = True):
        ...
        self.rolling_weight_contrib = torch.nn.Parameter(torch.zeros((num_gaussians, 1), dtype=torch.float32, device=self.device))
    ...
    def init_from_random_point_cloud(
        ...
        self.rolling_weight_contrib = torch.nn.Parameter(torch.zeros((num_gaussians, 1), dtype=dtype, device=self.device))
    ...
    def init_from_checkpoint(self, checkpoint: dict, setup_optimizer=True):
        ...
        self.rolling_weight_contrib = torch.nn.Parameter(torch.zeros(
    ...
    def default_initialize_from_points(self, pts, observer_pts, colors=None, use_observer_pts=True):
        ...
        self.features_albedo = torch.nn.Parameter(features_albedo.to(dtype=dtype, device=self.device))
        self.features_specular = torch.nn.Parameter(features_specular.to(dtype=dtype, device=self.device))
        self.rolling_weight_contrib = torch.nn.Parameter(torch.zeros((N, 1), dtype=dtype, device=self.device))
    ...
    def setup_optimizer(self, state_dict=None):
        ...
        # hijacking the params_update_fn to update the size of rolling weight contrib
        # params.append({"params": [self.rolling_weight_contrib], "name": "rolling_weight_contrib", "lr": 0.0}) 
    ...
    def set_optimizable_parameters(self):
        ...
        self.rolling_weight_contrib.requires_grad = False  # Always enable optimization of rolling weight contrib
    ...
    def init_from_ingp(self, ingp_path, init_model=True):
        ...
        self.rolling_weight_contrib = torch.nn.Parameter(torch.zeros((mog_num, 1), dtype=torch.float32, device=self.device))
    ...
    @torch.no_grad()
    def update_rolling_weight_contrib(self, weight_contrib):
        assert weight_contrib.shape == self.rolling_weight_contrib.shape, f"Expected shape {self.rolling_weight_contrib.shape}, but got {weight_contrib.shape}"
        self.rolling_weight_contrib += weight_contrib.detach()