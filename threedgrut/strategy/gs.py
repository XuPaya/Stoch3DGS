...
class GSStrategy(BaseStrategy):
    ...
    def densify_gaussians(self, scene_extent):
        ...
        self.model.rolling_weight_contrib = torch.nn.Parameter(torch.zeros((self.model.num_gaussians, 1), device=self.model.device))
    ...
    def prune_gaussians_opacity(self):
        ...
        # limit the number of gaussians to 3.0M. Remove the following block if you don't want to limit the number of gaussians. 
        # Note that the additional weight accumulation in referenceOptix.cu can also be removed for better performance.
        if self.model.num_gaussians > 3_000_000:
            weights = self.model.rolling_weight_contrib.squeeze()
            sorted_weights, _ = torch.sort(weights, descending=True)
            prune_weight_threshold = sorted_weights[2_700_000]
            if sorted_weights.sum() > 0: # don't prune if sorted weights is empty
                mask = torch.logical_and(mask, weights > prune_weight_threshold)
                print(f"Weight-based pruning to reach <3.0M gaussians: weight_threshold={prune_weight_threshold}")
